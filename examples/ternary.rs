...
fn main() {
    ...
    let (msg5, _) = Message::from_msgs(1, vec![&msg1, &msg2], &weights, None, None).unwrap();
    let (msg6, _) = Message::from_msgs(3, vec![&msg3, &msg4], &weights, None, None).unwrap();
    let (msg7, _) = Message::from_msgs(2, vec![&msg2, &msg5, &msg6], &weights, None, None).unwrap();
    let (msg8, _) = Message::from_msgs(3, vec![&msg7, &msg6], &weights, None, None).unwrap();
    let (msg9, _) = Message::from_msgs(4, vec![&msg4, &msg6], &weights, None, None).unwrap();
