...
mod tests {
    ...
    fn example_usage() {
        ...
        let proto_b1 = Block::new(None, None);
        ...
        let proto_b2 = Block::new(None, None);
        ...
        assert_eq!(
            m3.estimate(),
            &Block::new(Some(Block::from(&m2)), None),
            "should build on top of m2 as sender2 has more weight"
        );

        let proto_b4 = Block::new(None, None);
        let (m4, _) =
            BlockMsg::from_msgs(sender4, vec![&m1], &sender_state, Some(proto_b4), None).unwrap();

        assert_eq!(
            m4.estimate(),
            &Block::new(Some(Block::from(&m1)), None),
            "should build on top of m1 as as thats the only msg it saw"
        );

        let proto_b5 = Block::new(None, None);
        // println!("\n3 {:?}", m3);
        // println!("\n4 {:?}", m4);
        let (m5, _) =
        ...
        assert_eq!(
            m5.estimate(),
            &Block::new(Some(Block::from(&m3)), None),
            "should build on top of "
        );

        let block = Block::from(&m3);
        assert_eq!(block, Block::new(Some(Block::from(&m2)), None),);
    ...
    fn test2() {
        ...
        //     "genesis block with None as prevblock"
        // );
        let proto_b0 = Block::new(None, None);
        ...
        let proto_b1 = Block::new(None, None);
        let (m1, sender_state) =
            BlockMsg::from_msgs(sender1, vec![&m0], &sender_state, Some(proto_b1), None).unwrap();
        ...
        let proto_b3 = Block::new(None, None);
        let (m3, sender_state) =
            BlockMsg::from_msgs(sender3, vec![&m2], &sender_state, Some(proto_b3), None).unwrap();
        ...
        // println!("\n4 {:?}", m3);
        let (m4, sender_state) =
            BlockMsg::from_msgs(sender4, vec![&m2], &sender_state, Some(proto_b4), None).unwrap();
        // assert_eq!(
        //     m4.estimate(),
        ...
        assert_eq!(
            m5.estimate(),
            &Block::new(Some(Block::from(&m4)), None),
